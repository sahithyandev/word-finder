...
const words: Word[] = [
  ...
  },
  {
    word: "KOMMETJIE",
    clues: [
      "Small town in South Africa near Cape Town"
    ],
...
const fix = (origin: string, toFix: string, key: string) => {
  return toFix.split('').map((val, i) => origin[i] == key ? key : val).join('')
}
...
export class App {
  ...
  constructor() {
    this.selectedWord = words[random(words.length)] = words[random(words.length)];
  ...
  checkKey(key: string) {
    if (this.selectedWord.word.includes(key)) {
      ...
      this.selectedWord.showWord = fix(this.selectedWord.word, this.selectedWord.showWord, key);
      this.checkFinished()
    }
  }
